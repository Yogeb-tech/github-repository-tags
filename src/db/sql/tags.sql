-- INSERT tag
INSERT OR IGNORE INTO tags VALUES (?);  -- Only name needed
...
DELETE FROM tag_repositories WHERE tag_name = ? AND repo_url = ?;

-- GET tag usage count
SELECT t.name, COUNT(tr.repo_url) as usage_count
FROM tags t
